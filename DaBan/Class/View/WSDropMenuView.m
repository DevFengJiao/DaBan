...
#define KTableViewMaxHeight 450
...
+ (instancetype)twIndexPathWithColumn:(NSInteger)column
                                selec:(NSInteger)selec
                                  row:(NSInteger)row
                                 item:(NSInteger)item
                                 rank:(NSInteger)rank{
    
    WSIndexPath *indexPath = [[self alloc]initWithColumn:column selec:selec row:row item:item rank:rank];
...
- (UITableView *)leftTableView{
    ...
    if (!_leftTableView) {
        ...
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 40, 130, 380) style:UITableViewStylePlain];
        ...
        _leftTableView.backgroundColor = [UIColor colorWithRed:230/255.0 green:230/255.0 blue:230/255.0 alpha:1.0];
...
- (UITableView *)leftTableView_1{
    ...
    if (!_leftTableView_1) {
        ...
        _leftTableView_1 = [[UITableView alloc] initWithFrame:CGRectMake(130, 0, self.frame.size.width-130, self.frame.size.height) style:UITableViewStylePlain];
...
- (void)_showLeftTableViews{
    ...
    self.leftTableView.frame = CGRectMake(self.leftTableView.frame.origin.x, self.leftTableView.frame.origin.y, 130, KTableViewMaxHeight);
    self.leftTableView_1.frame = CGRectMake(self.leftTableView_1.frame.origin.x, self.leftTableView_1.frame.origin.y, SCREEN_WIDTH-110, KTableViewMaxHeight);
...
-(NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    WSIndexPath *twIndexPath =[self _getTwIndexPathForSeleWithtableView:tableView];
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(dropMenuView:selectWithIndexPath:)]) {
        
        NSInteger count =  [self.dataSource dropMenuView:self selectWithIndexPath:twIndexPath];
        _currSelect = count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView_1) {
        ...
        UILabel *labe = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 2.5, cell.frame.size.height)];
        labe.backgroundColor = BGVCOLOR;
        [cell.selectedBackgroundView addSubview:labe];
...
- (WSIndexPath *)_getTwIndexPathForSeleWithtableView:(UITableView *)tableView{
    if (tableView == self.leftTableView) {
        return [WSIndexPath twIndexPathWithColumn:_currSelectColumn selec:_currSelect row:WSNoFound item:WSNoFound rank:WSNoFound];
    }
    return 0;
}
...
- (WSIndexPath *)_getTwIndexPathForNumWithtableView:(UITableView *)tableView{
    ...
    if (tableView == self.leftTableView_1 && _currSelectRow != WSNoFound) {
        ...
        return [WSIndexPath twIndexPathWithColumn:_currSelectColumn selec:WSNoFound row:_currSelectRow item:WSNoFound rank:WSNoFound];
    }
    
    if (tableView == self.leftTableView_2 && _currSelectRow != WSNoFound && _currSelectItem != WSNoFound) {
        return [WSIndexPath twIndexPathWithColumn:_currSelectColumn selec:_currSelect row:_currSelectRow item:_currSelectItem  rank:WSNoFound];
    }
    
    if (tableView == self.rightTableView) {
        
        return [WSIndexPath twIndexPathWithColumn:1 selec:WSNoFound row:WSNoFound item:WSNoFound  rank:WSNoFound];
    }
...
- (WSIndexPath *)_getTwIndexPathForCellWithTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView) {
        ...
        return  [WSIndexPath twIndexPathWithColumn:0 selec:indexPath.section row:WSNoFound item:WSNoFound rank:WSNoFound];
    ...
    if (tableView == self.leftTableView_1) {
        ...
        return [WSIndexPath twIndexPathWithColumn:_currSelectColumn selec:_currSelect row:_currSelectRow item:indexPath.row rank:WSNoFound];
    }
    
    if (tableView == self.leftTableView_2) {
        return [WSIndexPath twIndexPathWithColumn:_currSelectColumn selec:_currSelect row:_currSelectRow item:_currSelectItem  rank:indexPath.row];
    }
    
    if (tableView == self.rightTableView) {
        
        return [WSIndexPath twIndexPathWithColumn:1 selec:indexPath.section row:indexPath.row item:WSNoFound  rank:WSNoFound];
    }
    
    
    return  [WSIndexPath twIndexPathWithColumn:0 selec:indexPath.section row:indexPath.row item:WSNoFound rank:WSNoFound];
