...
@interface XHMessageTableViewController ()<MAMapViewDelegate,LocationViewDelegate>
...
- (XHShareMenuView *)shareMenuView {
    if (!_shareMenuView) {
        XHShareMenuView *shareMenuView = [[XHShareMenuView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight-99)];
...
//    messageTableView.backgroundColor = BACK_GRAY_COLOR;
    ...
    // 设置整体背景颜色
    [self setBackgroundColor:BACK_GRAY_COLOR];
...
#pragma mark - XHShareMenuView Delegate(按钮点击事件)
...
- (void)didSelecteShareMenuItem:(XHShareMenuItem *)shareMenuItem atIndex:(NSInteger)index {
    ...
    switch (index) {
        ...
        case 3: {
//
            LocationViewController *locationController = [[LocationViewController alloc] initWithLocation:CLLocationCoordinate2DMake(_latitude,_longitude)];
...
-(void)sendLocationLatitude:(double)latitude longitude:(double)longitude andAddress:(NSString *)address
{
    ...
    WEAKSELF
    [weakSelf didSendGeolocationsMessageWithGeolocaltions:address location:location];
}
