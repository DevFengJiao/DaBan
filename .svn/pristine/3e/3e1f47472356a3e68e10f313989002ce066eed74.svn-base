...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            self.title = NSLocalizedString(@"选择图片", nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6.0")) {
                    ...
                    if ([MFMailComposeViewController canSendMail]) {
                        _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                               ...
                                                               otherButtonTitles:NSLocalizedString(@"保存", nil), NSLocalizedString(@"复制", nil), NSLocalizedString(@"Email", nil), nil];
                    } else {
                        _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                               cancelButtonTitle:NSLocalizedString(@"取消", nil) destructiveButtonTitle:nil
                                                               otherButtonTitles:NSLocalizedString(@"保存", nil), NSLocalizedString(@"复制", nil), nil];
                    }
...
- (void)actuallyCopyPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [self showProgressHUDCompleteMessage:NSLocalizedString(@"复制完成", @"Informing the user an item has finished copying")];
        [self hideControlsAfterDelay]; // Continue as normal...
    }
}
...
- (void)actuallyEmailPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [emailer setSubject:NSLocalizedString(@"图片", nil)];
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    if (result == MFMailComposeResultFailed) {
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"警告", nil)
                                                         message:NSLocalizedString(@"Email发送失败, 请重新尝试", nil)
